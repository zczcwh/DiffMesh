...
from lib.model.model_mesh_diff import MeshRegressor
from lib.utils.vismo import render_and_save, render_and_save_black
from lib.utils.utils_smpl import *
from scipy.optimize import least_squares

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="configs/mesh/DM_ft_pw3d.yaml", help="Path to the config file.")
    parser.add_argument('-e', '--evaluate', default='checkpoint/mesh/demo/best_epoch.bin', type=str, metavar='FILENAME', help='checkpoint to evaluate (file name)')
    ...
    parser.add_argument('--clip_len', type=int, default=16, help='clip length for network input')
...
with torch.no_grad():
    for batch_input in tqdm(test_loader):
        ...
        actual_frame = batch_input.shape[1]
        if batch_input.shape[1] != args.clip_len:
            pad_flag = True
            num_frame_less = batch_input.shape[1]
            feature_last_frame = batch_input[:,-1,:,:].unsqueeze(1).repeat(1,args.clip_len-batch_input.shape[1],1,1)
            batch_input = torch.cat((batch_input,feature_last_frame), dim=1)
        ...
        output_flip_back = [{
            'verts': output_flip_verts.reshape(batch_size, args.clip_len, -1, 3) * 1000.0,
            'kp_3d': output_flip_kp3d.reshape(batch_size, args.clip_len, -1, 3),
        }]
        output_final = [{}]
        ...
        verts_all.append(output[0]['verts'][:,:actual_frame].cpu().numpy())
        reg3d_all.append(output[0]['kp_3d'][:,:actual_frame].cpu().numpy())
...
render_and_save_black(verts_all, osp.join(opts.out_path, 'mesh.mp4'), keep_imgs=True, fps=fps_in, draw_face=True)
