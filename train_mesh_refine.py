...
from lib.data.dataset_mesh_refine import MotionSMPL
from lib.model.model_mesh_diff_refine import MeshRegressor
from torch.utils.data import DataLoader
import logging
...
def parse_args():
    ...
    parser.add_argument('-freq', '--print_freq', default=200)
...
def validate(test_loader, model, criterion, logger, dataset_name='h36m'):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for idx, (batch_input, batch_gt) in tqdm(enumerate(test_loader)):
            ...
            if torch.cuda.is_available():
                ...
                batch_gt['pred_smpl_shape'] = batch_gt['pred_smpl_shape'].cuda().float()
                batch_gt['pred_smpl_pose'] = batch_gt['pred_smpl_pose'].cuda().float()
                batch_input = batch_input.cuda().float()
            output = model(batch_input, init_pose=batch_gt['pred_smpl_pose'], init_shape=batch_gt['pred_smpl_shape'])
            output_final = output
            if args.flip:
                batch_input_flip = flip_data(batch_input)
                init_pose_flip = flip_thetas_batch(batch_gt['pred_smpl_pose'])
                output_flip = model(batch_input_flip, init_pose=init_pose_flip, init_shape=batch_gt['pred_smpl_shape'])
...
def train_epoch(args, opts, model, train_loader, losses_train, losses_dict, mpjpes, mpves, criterion, optimizer,
    ...
    for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader)):
        ...
        loss_train = args.lambda_3d * loss_dict['loss_3d_pos'] + \
                     ...
                     args.lambda_pose * loss_dict['loss_pose'] + \
                     args.lambda_norm * loss_dict['loss_norm'] + \
                     args.lambda_diff * loss_dict['loss_diff']
...
def train_with_config(args, opts):
    ...
    log_path = os.path.join(opts.checkpoint, 'results_epoch.log')
        ...
        for epoch in range(st, args.epochs):
            ...
            if hasattr(args, "dt_file_pw3d"):
                ...
                test_loss_pw3d, test_mpjpe_pw3d, test_pa_mpjpe_pw3d, test_mpve_pw3d, test_losses_dict_pw3d = validate(
                    test_loader_pw3d, model, criterion, logger,'pw3d')
    ...
    if opts.evaluate:
        if hasattr(args, "dt_file_h36m"):
            test_loss, test_mpjpe, test_pa_mpjpe, test_mpve, _ = validate(test_loader, model, criterion, logger,'h36m')
        if hasattr(args, "dt_file_pw3d"):
            test_loss_pw3d, test_mpjpe_pw3d, test_pa_mpjpe_pw3d, test_mpve_pw3d, _ = validate(test_loader_pw3d, model,
                                                                                              criterion, logger,'pw3d')
