...
class MeshLoss(nn.Module):
    ...
    def forward(
        ...
        if 'loss_diff' in preds:
            seq = preds['loss_diff'].shape[1] // 2
            noise, pred_noise = preds['loss_diff'][:,:seq], preds['loss_diff'][:,seq:]
            noise = reduce(noise)
            pred_noise = reduce(pred_noise)
            loss_diff = torch.mean(torch.norm(noise - pred_noise, dim=len(pred_noise.shape)-1))
