...
def render_and_save_black(motion_input, save_path, keep_imgs=False, fps=25, color="#F96706#FB8D43#FDB381", with_conf=False, draw_face=False):
    ensure_dir(os.path.dirname(save_path))
    motion = copy.deepcopy(motion_input)
    if motion.shape[-1]==2 or motion.shape[-1]==3:
        motion = np.transpose(motion, (1,2,0))   #(T,17,D) -> (17,D,T)
    if motion.shape[1]==2 or with_conf:
        ...
        # motion_world = pixel2world_vis_motion(motion, dim=3)
        motion2video_mesh_black(motion, save_path=save_path, keep_imgs=keep_imgs, fps=fps, draw_face=draw_face)
    else:
        motion_world = pixel2world_vis_motion(motion, dim=3)
        motion2video_3d(motion_world, save_path=save_path, keep_imgs=keep_imgs, fps=fps)

def render_and_save_black_mb(motion_input, save_path, keep_imgs=False, fps=25, color="#F96706#FB8D43#FDB381", with_conf=False, draw_face=False):
    ensure_dir(os.path.dirname(save_path))
    motion = copy.deepcopy(motion_input)
    if motion.shape[-1]==2 or motion.shape[-1]==3:
        motion = np.transpose(motion, (1,2,0))   #(T,17,D) -> (17,D,T)
    if motion.shape[1]==2 or with_conf:
        ...
        # motion_world = pixel2world_vis_motion(motion, dim=3)
        motion2video_mesh_black_mb(motion, save_path=save_path, keep_imgs=keep_imgs, fps=fps, draw_face=draw_face)
    else:
        motion_world = pixel2world_vis_motion(motion, dim=3)
        motion2video_3d(motion_world, save_path=save_path, keep_imgs=keep_imgs, fps=fps)
...
def motion2video_mesh_black(motion, save_path, fps=25, keep_imgs=False, draw_face=True):
    videowriter = imageio.get_writer(save_path, fps=fps)
    vlen = motion.shape[-1]
    draw_skele = (motion.shape[0] == 17)
    save_name = save_path.split('.')[0]
    ...
    for f in tqdm(range(vlen)):
        ...
        if draw_skele:
            for i in range(len(joint_pairs)):
                limb = joint_pairs[i]
                xs, ys, zs = [np.array([j3d[limb[0], j], j3d[limb[1], j]]) for j in range(3)]
                ax.plot(-xs, -zs, -ys, c=[0, 0, 0], lw=3, marker='o', markerfacecolor='w', markersize=3,
                        markeredgewidth=2)  # axis transformation for visualization
        elif draw_face:
            # ax.plot_trisurf(j3d[:, 0], j3d[:, 1], triangles=smpl_faces, Z=j3d[:, 2],
            #                 color=(255 / 255.0, 255 / 255.0, 255 / 255.0, 0.9))    ### (255 / 255.0, 255 / 255.0, 255 / 255.0, 0.9))  (166 / 255.0, 188 / 255.0, 218 / 255.0, 0.9))
            ax.plot_trisurf(j3d[:, 0], j3d[:, 1], triangles=smpl_faces, Z=j3d[:, 2],
...
def motion2video_mesh_black_mb(motion, save_path, fps=25, keep_imgs=False, draw_face=True):
    videowriter = imageio.get_writer(save_path, fps=fps)
    vlen = motion.shape[-1]
    draw_skele = (motion.shape[0] == 17)
    save_name = save_path.split('.')[0]
