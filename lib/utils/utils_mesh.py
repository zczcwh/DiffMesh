...
def evaluate_mesh_acc(results):
    ...
    ##########
    num_round = pred_j3ds.shape[0]//16
    accel_err = np.zeros((num_round, ))
    pred_j3ds_acc = pred_j3ds.reshape(num_round, 16, 14, 3)
    target_j3ds_acc = target_j3ds.reshape(num_round, 16, 14, 3)
    for i in range(num_round):
        accel_err_round = np.zeros((16,))
        accel_err_round[1:-1] = compute_error_accel(joints_pred=pred_j3ds_acc[i], joints_gt=target_j3ds_acc[i])
        accel_err[i]=accel_err_round[1:-1].mean()
